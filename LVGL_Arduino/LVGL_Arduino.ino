...
void sendAudio() {
  const char* host = "192.168.137.194";
  ...
  client.print("Connection: close\r\n");  // Important to close connection after response
  client.print("\r\n");
  ...
  while (client.connected() && millis() < timeout) {
    while (client.available()) {
      ...
      if (!headersEnded) {
        if (line == "") {
          headersEnded = true;  // End of HTTP headers
        }
      } else {
        response += line + "\n";
      }
...
bool writeResults(String result){

  Serial.println("Raw response string:");
  Serial.println(result);
  StaticJsonDocument<1024> doc;
  DeserializationError error = deserializeJson(doc, result);
  if(error){
    Serial.print("Error deserializing\n");
  }
  ...
  const char* result2=doc["result"];

  //time,age,gender,emotion,result
  File file = SD_MMC.open("/results.txt", FILE_APPEND);
if (!file) {
  Serial.println("results.txt does not exist, creating...");
  file = SD_MMC.open("/results.txt", FILE_WRITE);
}
if (!file) {
  Serial.println("Still cannot open results.txt for writing");
  return false;
}
  ...
  if(!file){
    ...
    file.printf("%s,%s,%s,%s,%s\n", time, age, gender, emotion, result2);
    file.close();
    Serial.println("Wrote results succesfuly in results.txt");
    return true;
  }
